...
class MaintenanceExecutor:
    ...
    async def apply(self, report: dict[str, Any]) -> None:
        ...
        if not triggers:
            self._state.set(
                DegradationLevel.NORMAL, source="trainer.maintenance", reason="healthy"
            )
            return
        ...
        if mode.action == "pause":
            self._state.set(
                DegradationLevel.PAUSED, source="trainer.maintenance", reason=reason
            )
        elif mode.action == "degrade":
            self._state.set(
                DegradationLevel.DEGRADED, source="trainer.maintenance", reason=reason
            )
        # "alert_only" — do not change state, just notify.

        message = (
            f"Trainer maintenance triggered: {', '.join(triggers)}; action={mode.action}"
        )
