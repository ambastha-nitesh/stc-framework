...
class HallucinationValidator(Validator):
    ...
    def _validate_overlap(self, ctx: ValidationContext, sentences: list[str]) -> GuardrailResult:
        ...
        return GuardrailResult(
            ...
            details=(f"Grounding score: {grounding_score:.2f} " f"(threshold: {self._threshold})"),
            evidence={
    ...
    async def _validate_embedding(self, ctx: ValidationContext, sentences: list[str]) -> GuardrailResult:
        import math
        ...
        return GuardrailResult(
            ...
            severity=self.severity if not passed else "low",
            action="pass" if passed else "block",
            details=(f"Embedding grounding score: {grounding_score:.2f} " f"(threshold: {self._threshold})"),
            evidence={
                "grounding_score": grounding_score,
                "total_sentences": len(sentences),
                "ungrounded_sentences": ungrounded[:5],
