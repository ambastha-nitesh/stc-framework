...
## Phase 2 — seed the human-seeded secrets

Terraform creates Secrets Manager entries for **every** secret, but
only populates the ones it can generate itself. Split into two
categories:

- **Terraform-managed** (nothing to do — populated during `apply`):
  - `${NP}-redis_auth_token` — generated by `random_password` inside
    the `elasticache` module.
  - `${NP}-redis_url` — composed in the root module once Redis is up.
  - Aurora master password — managed natively by RDS via
    `manage_master_user_password`; the generated secret lives at an
    AWS-named ARN exposed through `rds_master_user_secret_arn`.
- **Human-seeded** (set these BEFORE the first `apply` — the service
  task will crash without them):
...
aws secretsmanager put-secret-value --secret-id ${NP}-litellm_api_key \
  --secret-string "sk-..."
# Qdrant API key (optional; only if you turned on qdrant auth)
aws secretsmanager put-secret-value --secret-id ${NP}-qdrant_api_key \
  --secret-string "qdrant-..."
```
